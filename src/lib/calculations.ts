...
export function calculateRow(row: ReworkRow, rates: WageRates): RowCalculation {
  ...
  const supervisorCost =
    row.supervisorPeople * row.supervisorHours * rates.supervisor;
...
export function calculateTotals(report: ReportState): ReportTotals {
  return report.rows.reduce<ReportTotals>(
    (totals, row) => {
      ...
      if (row.workers > 0 && row.normalHours + row.otHours > 0)
        totals.workedDays += 1;
      if (row.supervisorPeople > 0 && row.supervisorHours > 0)
        totals.supervisorDays += 1;
      return totals;
    },
    {
      employeeHours: 0,
